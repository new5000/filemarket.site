    ...
    <div className={`min-h-screen ${darkMode ? 'dark bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'} transition-colors duration-200 flex flex-col font-sans`}>
      ...
      <div className="flex flex-1 relative">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-3 sm:px-6 py-2.5 sm:py-3 sticky top-0 z-30 flex items-center justify-between gap-2 shadow-xs">
            <div className="min-w-0 flex-1">
              {/* Breadcrumb Path */}
              <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                <span className="hover:text-slate-700 dark:hover:text-slate-200 cursor-pointer hidden sm:inline" onClick={() => handleTabChange('dashboard')}>Admin Hub</span>
                <ChevronRight className="w-3.5 h-3.5 opacity-60 hidden sm:inline" />
                <span className="font-bold text-slate-900 dark:text-white truncate">{currentNav.shortLabel || currentNav.label}</span>
              </div>
            </div>

            {/* Quick Status Bar */}
            <div className="flex items-center gap-1.5 shrink-0">
              <div className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[11px] font-bold">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
              ...
              <button
                onClick={handleExitToStore}
                className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-[11px] font-bold transition cursor-pointer"
              >
                <span>Store</span>
