...
export const AdminUsersView: React.FC<AdminUsersViewProps> = ({ users: initialUsers = [], onRefresh }) => {
  ...
  const [userToBlock, setUserToBlock] = useState<{ user: any; newStatus: string } | null>(null);
  const [orderToDelete, setOrderToDelete] = useState<{ orderId: string; productId: string; userId: string; title: string } | null>(null);
  ...
  // 1. Realtime Firestore Listeners (onSnapshot) for User Inspector Modal (Orders + Direct Locker Assets)
  useEffect(() => {
    ...
    let rawOrdersList: any[] = [];
    let profileAssetsList: any[] = [];
    let subcollectionLockerList: any[] = [];
      ...
      try {
        ...
        const unsubscribeUser = onSnapshot(userDocRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            const combinedArrays: any[] = [...purchases];
  ...
  // Toggle User Block Status (Prompt)
  ...
  // Revoke User Product Access & Delete Order with Atomic Firestore Batch
  const executeRevokeUserProductAccess = async (orderId?: string, productId?: string, userId?: string) => {
    ...
    try {
      ...
      if (selectedUser && selectedUser.purchasedProducts && Array.isArray(selectedUser.purchasedProducts)) {
        const updatedPurchases = selectedUser.purchasedProducts.filter(
          (p: any) => p.id !== cleanOrderId && p.id !== cleanProductId && p.productId !== cleanProductId
        );
  ...
  const handlePromptDeleteOrder = (orderId?: string, productId?: string, userId?: string, title?: string) => {
    const cleanOrderId = String(orderId || '').trim();
    const cleanProductId = String(productId || cleanOrderId).trim();
    const targetUserId = String(userId || selectedUser?.id || selectedUser?.userId || selectedUser?.uid || '').trim();
  ...
  const handleDeleteUserOrder = (orderId: string) => {
  ...
  useEffect(() => {
    ...
    const handleContainerClick = (e: MouseEvent) => {
      ...
      const deleteBtn = target.closest('.delete-order-record-btn, .delete-order-btn') as HTMLElement | null;
      if (!deleteBtn) return;
  ...
  const lockerAssetsCount = (() => {
    ...
    if (selectedUser?.purchasedProducts && Array.isArray(selectedUser.purchasedProducts)) {
      selectedUser.purchasedProducts.forEach((p: any) => {
  ...
  return (
    <div className="space-y-6 text-slate-900 dark:text-slate-100 animate-in fade-in duration-300">
      ...
      {/* Master Admin Access Settings Component */}
